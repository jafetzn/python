...
def formatMethod(s):
    newText = "This is the text we added to the original"
    print(f"{s} We can also af text in here. {newText}")
...
def moreFormat1():
    name = "Pao"
    print("Hi your lucky number is {number}, {name}".format(name=name, number=len(name)*3))

    #more format examples
    price=7.5
    with_tax=price*1.09
    print("Base price:${:.2f}. WIth tax ${:.3f}".format(price, with_tax))


def moreFormatAdvanced():
    #f the placeholders indicate a number, they’re replaced by the variable corresponding to that order (starting at zero).
